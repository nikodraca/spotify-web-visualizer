...
const startListening = () => {
  ...
  const callback = (mutationsList, observer) => {
    for (let mutation of mutationsList) {
      if (mutation.type === 'attributes' && mutation.target.className === 'cover-art-image cover-art-image-loaded') {
...
const fetchCurrentlyPlaying = () => {
  const newSong = {
    title: $('.track-info__name')
      .first()
      .text(),
    artist: $('.track-info__artists')
      .first()
      .text(),
    image_url: $('.now-playing__cover-art .cover-art .cover-art-image')
      .css('background-image')
      .slice(5, -2),
    href: $('.track-info__name .react-contextmenu-wrapper span a').attr('href')
  };

  // check to see if different song
  if (!CURRENT_SONG || CURRENT_SONG.href !== newSong.href) {
...
const updateVisualizer = () => {
  ...
  $('#visualizer').html(`
	...
	<img src="${CURRENT_SONG.image_url}" class="visualizer-img"></img>

	<img src="${CURRENT_SONG.image_url}" class="song-img"></img>
	<h1 class="visualizer-text">${CURRENT_SONG.title}</h1>
	<h3 class="visualizer-text">${CURRENT_SONG.artist}</h3>
  `);
