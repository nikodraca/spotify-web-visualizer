...
const startListening = () => {
  ...
  const callback = (mutationsList, observer) => {
    for (let mutation of mutationsList) {
      if (mutation.type === 'attributes' && mutation.target.className === 'cover-art-image') {
        fetchCurrentlyPlaying();
      }
    }
  };
...
const fetchCurrentlyPlaying = () => {
  ...
  const imageUrlStyle = nowPlaying
    .find('.cover-art-image')
    .first()
  ...
  const imageUrl = /\("([^)]+)"\)/i.exec(imageUrlStyle)[1];
...
const updateVisualizer = () => {
  ...
  $('#visualizer').html(`
    ...
    <img src="${imageUrl}" class="song-img"></img>
